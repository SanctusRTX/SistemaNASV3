...
function configurarBotonesEliminarEspecial(intentos) {
    // Limitar a 3 intentos para evitar loop infinito
    intentos = intentos || 0;
    if (intentos >= 3) {
        console.log("[eliminar.js] Sin botones .btn-danger para configurar, deteniendo.");
        return;
    }

    // Seleccionar solo botones con data-ruta y data-nombre para no afectar otros módulos
    var botones = document.querySelectorAll(".btn-eliminar[data-ruta]");
    var contador = 0;
    ...
    console.log("[eliminar.js] Configurados " + contador + " botones de eliminar.");
    
    if (contador === 0 && intentos < 2) {
        setTimeout(function() { configurarBotonesEliminarEspecial(intentos + 1); }, 1000);
    }
}
